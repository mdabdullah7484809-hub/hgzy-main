...
const Gamesdetails = () => {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        const response = await axios.get(`https://wingobd.onrender.com/admin/game/id/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://wingobd.onrender.com/admin/update-game/${id}`, data);
        ...
        <form onSubmit={handleSubmit} className="bg-white shadow-lg rounded-xl p-6">
          ...
          <div className="mb-6">
            ...
            >
              {formData.image ? (
                <img
                  src={formData.image instanceof File ? URL.createObjectURL(formData.image) : `https://wingobd.onrender.com/images/${formData.image}`}
                  alt="Uploaded"
